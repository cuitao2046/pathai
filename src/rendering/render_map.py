...
school_building_01_map_v9.geojson：geometry / semantic / topology 三段式），
为每个楼层渲染平面图 PNG，并支持叠加导航拓扑（节点/边）的调试图。

用法:
    python render_map.py [geojson_path] [--topology]

参数:
    geojson_path   输入 GeoJSON，默认 result/school_building_01_map_v9.geojson
...
DEFAULT_GEOJSON = str(
    Path(__file__).resolve().parent.parent.parent / "result" / "school_building_01_map_v9.geojson")
...
DOOR_OPENING_COLOR = "#1E8449"  # 通道门洞(DK 洞口/出入口/公共空间) 绿
...
NODE_COLORS = {
    "room": "#E67E22",                 # 房间
    "doorway": "#C0392B",             # 门口
    "intersection": "#27AE60",        # 走廊交叉口
    "facility_entrance": "#2980B9",   # 建筑/无障碍出入口
}
# facility 由 facilityType 决定颜色（楼梯紫 / 电梯青）
NODE_MARKERS = {
    "room": "o",
    "doorway": "o",
    "intersection": "s",
...
def draw_doors(ax, doors):
    ...
    三种门洞区分：
      - swing   平开门：蓝色圆环
      - fire    防火门：红色方块
      - opening 通道门洞（DK 洞口/建筑出入口/公共空间，无摆弧）：绿色空心菱形
    """
    for dr in doors:
        ...
        if kind == "fire":
            ...
            ax.add_patch(MplPolygon(diamond, closed=True, facecolor="white",
                                    edgecolor=DOOR_OPENING_COLOR, lw=1.4,
                                    zorder=7))
        else:
...
def draw_topology(ax, topology, cross_floor_edges=None):
    """导航拓扑叠加。

    边为绿色细线（按 from/to 节点 ID 查坐标，含同层所有边）。
    跨层边（root.crossFloorEdges）的端点仅其一在本层，用橙色/蓝色双环标记
    竖向可达接入点（楼梯橙 / 电梯蓝）。
    节点按类型着色：房间橙、门口红、走廊交叉口绿、设施紫(楼梯)/青(电梯)、
    出入口蓝。
    """
    ...
    for e in topology.get("edges", []):
        ...
        if not a or not b:
            continue
        ax.plot([a["coordinates"][0], b["coordinates"][0]],
                [a["coordinates"][1], b["coordinates"][1]],
                color="#27AE60", lw=0.7, alpha=0.6, zorder=9)

    # ---- 跨层边（仅绘制本层端点，标记竖向接入） ----
    for e in (cross_floor_edges or []):
        a = nodes.get(e.get("from"))
        b = nodes.get(e.get("to"))
        present = a or b
        if not present:
            continue
        x, y = present["coordinates"]
        color = "#E67E22" if e.get("type") == "staircase" else "#2980B9"
        ax.add_patch(Circle((x, y), 1.7, facecolor="none", edgecolor=color,
                            lw=1.7, zorder=11))
        ax.add_patch(Circle((x, y), 0.9, facecolor="none", edgecolor=color,
                            lw=1.2, zorder=11))
    ...
    for n in topology.get("nodes", []):
        ...
        if n.get("type") == "facility":
            color = "#8E44AD" if n.get("facilityType") == "staircase" else "#16A085"
        else:
            color = NODE_COLORS.get(n.get("type"), "#7F8C8D")
        marker = NODE_MARKERS.get(n.get("type"), "o")
        ms = NODE_SIZES.get(n.get("type"), 4.0)
        ax.plot(x, y, marker, ms=ms, color=color, zorder=10,
...
def draw_decorations(ax, bounds, title, legend_x):
    ...
    handles = [
        ...
        Line2D([0], [0], marker="D", color="none", markerfacecolor="white",
               markeredgecolor=DOOR_OPENING_COLOR, markeredgewidth=1.4,
               markersize=8, label="通道门洞(DK)"),
...
def render_floor(floor_id, floor, venue_name, out_dir, with_topology,
    ...
    if with_topology:
        ...
        draw_topology(ax2, floor.get("topology", {}),
                      cross_floor_edges=cross_floor_edges)
        ax2.set_title(f"{venue_name} · {floor_id}F 导航拓扑", fontsize=15,
                      pad=12)
        # 拓扑图例（右上角预留带）
        topo_handles = [
            Line2D([0], [0], color="#27AE60", lw=1.2, label="导航边"),
            Line2D([0], [0], marker="o", color="none", markerfacecolor="#E67E22",
                   markeredgecolor="white", markeredgewidth=0.4, markersize=7,
                   label="房间节点"),
            Line2D([0], [0], marker="o", color="none", markerfacecolor="#C0392B",
                   markeredgecolor="white", markeredgewidth=0.4, markersize=6,
                   label="门口节点"),
            Line2D([0], [0], marker="s", color="none", markerfacecolor="#27AE60",
                   markeredgecolor="white", markeredgewidth=0.4, markersize=7,
                   label="走廊交叉口"),
            Line2D([0], [0], marker="P", color="none", markerfacecolor="#8E44AD",
                   markeredgecolor="white", markeredgewidth=0.4, markersize=8,
                   label="楼梯接入"),
            Line2D([0], [0], marker="P", color="none", markerfacecolor="#16A085",
                   markeredgecolor="white", markeredgewidth=0.4, markersize=8,
                   label="电梯接入"),
            Line2D([0], [0], marker="^", color="none", markerfacecolor="#2980B9",
                   markeredgecolor="white", markeredgewidth=0.4, markersize=8,
                   label="出入口"),
        ]
        ax2.legend(handles=topo_handles, loc="upper right",
                   bbox_to_anchor=(1.0, 1.0), bbox_transform=ax2.transAxes,
...
def main():
    ...
    cross_floor_edges = data.get("crossFloorEdges", [])
