...
- ⚠️ **push 凭据已通（2026-08-10，重克隆后 2026-08-11 复验）**：SSH 推送通道 = `~/.ssh/config` 的 `Host github-cuitao`（HostName github.com + IdentityFile `~/.ssh/id_rsa_cuitao` + IdentitiesOnly yes），已验证认证 `Hi cuitao2046!`。**remote 必须用 SSH 格式 `git@github-cuitao:cuitao2046/pathai.git`**，本机直接 `git push` 即可，无需 PAT/用户名密码。⚠️ 重新 `git clone https://...` 后 remote 会变回 https 格式，https push 需交互式凭据、沙箱无终端必失败——**clone 后第一件事：`git remote set-url origin git@github-cuitao:cuitao2046/pathai.git`**，不要因"沙箱无凭据"误以为不能推。
