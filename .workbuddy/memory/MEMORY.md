...
- ⚠️ **push 凭据（2026-08-14 修正）**：SSH 推送通道 = `~/.ssh/config` 的 `Host github-cuitao`（HostName github.com + IdentityFile `~/.ssh/id_ed25519` + IdentitiesOnly yes），已验证认证 `Hi cuitao2046!`。⚠️ **`id_rsa_cuitao` 密钥文件已不存在（2026-08-14 发现）**，可认证密钥是 `~/.ssh/id_ed25519`（`id_rsa` 不行），config 已于当日修正（备份 `~/.ssh/config.bak`）。**remote 必须用 SSH 格式 `git@github-cuitao:cuitao2046/pathai.git`**，本机直接 `git push` 即可，无需 PAT/用户名密码。⚠️ 重新 `git clone https://...` 后 remote 会变回 https 格式，https push 需交互式凭据、沙箱无终端必失败——**clone 后第一件事：`git remote set-url origin git@github-cuitao:cuitao2046/pathai.git`**，不要因"沙箱无凭据"误以为不能推。
...
✅ **用户已明确采纳（2026-08-12）**：确认双会话并行直推 master 会触发反复 rebase 冲突的风险，后续严格按本铁律——每个功能/优化/bugfix 一律从最新 master 切新分支（无斜杠名）再工作，**禁止双会话并行直推 master**。
