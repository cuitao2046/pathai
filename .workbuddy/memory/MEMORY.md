...
7. **开放空间与封闭空间分治（2026-08-05 数据层+拓扑层根治）**：走廊/门厅/大厅/活动(开放) 与 房间/管井/电梯/楼梯间(封闭) 是不同类型空间，**不得合并处理**。
   - **数据层**（`parse_cad_pdf.py`）：①`extract_room_labels` 的 `_cat` 新增 `"open"` 类，垂直标签合并只在同类内进行（避免「走道」+「历史地理资料室」拼成 `走道历史地理资料室`）；②`build_rooms` 多标签连通域对「开放∩封闭≠∅」走多源测地分割，各自独立成空间；③同类型多标签共域取**字号最大者**（ANNO-TEXT 层）为语义层；④开放空间房间 id 改用 **CR-/LB-/AC-/AT-** 前缀（`OBJ_TYPE` 增 4 键），不再混用 `RM-`。
   - **拓扑层**（`topology.py`）：新增 `OPEN_SPACE_TYPES={corridor,lobby,activity,atrium}`；房间节点循环跳过开放空间（仅封闭空间建 `topo_room` 节点），开放空间统一建 `intersection` 型 circulation 节点并携带 `roomType`；边 #1 跳过开放空间；新增兜底边 #6（无邻边时连最近门口→退一步连最近封闭房间）确保无孤岛。渲染层走廊画虚线轮廓（不填色）避免覆盖下方图层；intersection 节点详情面板显示中文「空间类型」。
