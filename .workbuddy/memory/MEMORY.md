...
7. **开放空间与封闭空间分治（2026-08-05 数据层根治）**：走廊/门厅/大厅/活动(开放) 与 房间/管井/电梯/楼梯间(封闭) 是不同类型空间，**不得合并处理**。解析器 `parse_cad_pdf.py` 已：①`extract_room_labels` 的 `_cat` 新增 `"open"` 类（走道/走廊/门厅/大厅/活动/社团/庭园/上空/出入口等），垂直标签合并只在同类内进行（避免「走道」+「历史地理资料室」拼成 `走道历史地理资料室`）；②`build_rooms` 多标签连通域对「开放∩封闭≠∅」走多源测地分割（同 shaft+toilet 逻辑），各自独立成空间；③同类型多标签共域取**字号最大者**为语义层；④开放空间房间 id 改用 **CR-/LB-/AC-/AT-** 前缀（`OBJ_TYPE` 在 `topology.py` 增 4 键），不再混用 `RM-`。渲染层走廊仍画虚线轮廓（不填色）避免覆盖下方图层。
