...
> **⚠️ 设计态文档**：本文为**设计文档**，`src/` 中**尚无对应实现模块**（无定位融合 / 卡尔曼滤波 / 指纹匹配代码）。当前已完成的验证均为**离线仿真**（如 `debug/trilateration_checker.py`、`debug/pf_imu_coupled.py`、`debug/assess_online_feasibility.py`，结论见 `docs/16-纯三点定位亚米级误差优化方案.md`）。因此，本文中的模型参数（采集间距、K 值、检索半径、卡尔曼噪声、置信度阈值、性能指标等）均为**目标值而非实测值**，落地前需以实测为准。
...
若匹配路段连接到高风险节点（楼梯、电梯厅），立即在该节点周边启用 0.5 米级精度的局部指纹子库重新匹配，确保安全提示的触发位置准确。
