...
def build_progress_report():
    ...
    para(doc, "为支撑现场实施，需采购测距定位设备、蓝牙信标与辅助耗材，详见附件《工具材料采购预算说明》。", after=2)
    bullet(doc, "按当前 61 枚部署 + 10 枚备用 = 71 枚计：激光测距套装约 ¥614；蓝牙信标三档备选 ¥1,917~3,195；辅助耗材 ¥180；采集终端与充电宝使用现有设备，不另购。方案 I 合计约 ¥2,711~3,989。")
    bullet(doc, "辅助耗材 ¥180 均为现场作业必需，直接支撑数据可信度：① 编号标签（¥30）为 71 枚信标贴实物编号、并标记 647 个采集点的已采/未采，是部署台账与指纹库点位完整性的核对依据；② A1 平面图纸 2 张（¥100，单层外廓约 76 m × 118 m，1:150~1:200 出图）作为现场按图作业与「图纸—实建差异」回写的基准；③ 杂项含记号笔与免钻孔固定材料（¥50）——现行 61 枚信标中 54 枚固定于墙面、7 枚固定于门套，均为校方资产不得钻孔，且需靠临时试挂实现点位微调。")
    bullet(doc, "采集终端复用成员现有手机：采集小程序为微信小程序，Android/iOS 均可完成 RSSI 采集；指纹库的距离真值由测距设备现场测得，不依赖手机传感器。")
...
def build_budget():
    ...
    bullet(doc, "为支撑初中学部 1# 教学楼 1~2 层视障导航系统的现场实施，需采购：①指纹采集定位设备（用于建立坐标真值）；②蓝牙信标（定位基础设施）；③辅助耗材。采集终端与充电使用现有设备，不列入采购。")
    ...
    make_table(doc,
        ...
        [["A1", "激光测距仪", "纯距离，±2mm（得力/优利德/米家）", "169", "1", "169", "测垂距与沿墙距离"],
         ["A2", "全景云台", "带水平刻度环+双水平泡", "200", "1", "200", "固定激光尺+水平旋转扫描"],
         ["A3", "设备夹具/快装板", "夹持激光尺机身", "30", "1", "30", "云台固定"],
         ["A4", "三脚架", "铝合金，1/4螺口，最高1.68m，承重4kg", "190", "1", "190", "架设于锚点，承载A2云台"],
         ["A5", "卷尺", "5-10m", "25", "1", "25", "沿墙量距"],
         ["", "合计", "", "", "", "614", ""]],
        widths=[1.0, 2.6, 4.3, 1.8, 1.2, 1.8, 3.3])
    para(doc, "测距仪 1 台即可：直角坐标法在同一测点上分两次测量（先量沿墙距离、再垂直打墙），一台顺序完成即可，不需同时工作；并与信标现场定位、地图勘测共用（docs/08 §6.3 同一口径）。若后续需双人并行采集或热备份，可再加购一台（+¥169）。京东在售参考价：A1 得力 50m±2mm 测距仪约 ¥150–170（如 100m 款 ¥154 / 50m 款 DL331040B）；A2 全景刻度云台约 ¥110–200（轻装时代 Q02 约 ¥112，1000+ 评论）；A3 云台快装板（1/4 螺口）约 ¥20–40（轻装时代原装快装板约 ¥38，2000+ 评论）；A4 云腾 VT-7008 三脚架（1/4 螺口、最高 1.68 m，承载 A2 云台；锚点架设需 1.2 m 以上工作高度，迷你/桌面型不可用）约 ¥164–214（云腾京东自营旗舰店，20 万+ 评论）；A5 得力 8210 钢卷尺 10m（京东自营）约 ¥22–28（200 万+ 评论）。以上搜索入口与在售状态已于 2026-09-21 逐项核对；上表单价为预算估算值，实际以下单时为准。", size=9, after=4)
    ...
    heading(doc, "C. 采集终端与充电（不列入采购）", 2)
    make_table(doc,
        ["序号", "物品", "说明"],
        [["C1", "采集终端", "使用成员现有手机，不采购（¥0）。采集小程序为微信小程序，Android/iOS 均可完成 RSSI 采集；距离真值由 A 类测距设备现场测得，不依赖手机传感器。"],
         ["C2", "充电宝", "自备，不采购（¥0）。现场采集约 3.3h，自有充电宝即可（原列 ¥100 已移除）。"]],
        widths=[1.2, 2.4, 11.4])
    para(doc, "信标批量配置可用厂商免费 App（MinewBeaconAdmin）在现有手机上完成，不另计费。", size=9, after=4)

    heading(doc, "D. 辅助耗材（现场作业必需）", 2)
    make_table(doc,
        ["序号", "物品", "单价(¥)", "数量", "小计(¥)", "使用环节", "用途"],
        [["D1", "编号标签/标记贴纸", "30", "1", "30", "②信标安装 ③覆盖验收 ④指纹采集", "信标实物编号（对台账）+ 采集点临时标记"],
         ["D2", "平面图纸打印（A1，1~2层各1张）", "50", "2", "100", "②信标安装 ④指纹采集 ⑥现场复核", "现场按图作业、按图标注"],
         ["D3", "杂项（记号笔、美纹胶带、扎带/魔术贴等）", "50", "1", "50", "②信标安装 ③覆盖验收", "图纸标注 + 信标试挂与免钻孔固定"],
         ["", "合计", "", "", "180", "", ""]],
        widths=[1.0, 4.2, 1.5, 1.2, 1.5, 3.6, 4.0])
    para(doc, "D1 编号标签（¥30）——数据可追溯性的最低硬件要求。为 71 枚信标逐枚贴物理编号，与部署台账 beaconId 一一对应（现行编号体系 BK-01-xxx / BK-02-xxx / BK-M-xxx）；并在 647 个指纹采集点（F1 611 / F2 36）贴可移除标记区分已采/未采。不采购的后果：安装后台账与实物无法对应，覆盖验收、后续更换电池（CR2477，标称 5 年）或点位微调时无法定位到具体是哪一枚；采集阶段无法核对 647 点是否采齐，漏采/重采只能靠人工回忆，指纹库点位完整性失去可核查性。用可移除贴纸而非直接书写，是因为教学楼为校方公共资产，墙面门套不得留永久痕迹。", size=9, after=3)
    para(doc, "D2 平面图纸打印 A1 ×2（¥100）——现场作业的基准载体。1F、2F 各 1 张（单层外廓实测约 76 m × 118 m，按 1:150~1:200 出图，旋转后恰好容纳），现场用于：① 按图核对信标点位与挂装位置（挂高 2.2 m）；② 记录 647 点指纹采集的行进顺序；③ 发现图纸与实建不符处直接图上标注，回写数据集中的 surveyRequired 待勘测项。不采购的后果：现场失去与数字地图的对照基准——61 枚信标坐标与 647 个采集点全部来自 CAD 数字化结果，无图纸可比对就无法发现「图纸与实建差异」这一已知风险；且采集顺序无法预先规划，显著增加现场往返与漏采。纸张可任意标注、不怕断电、能一眼看全整层拓扑，手机屏幕需反复缩放平移，不能替代。", size=9, after=3)
    para(doc, "D3 杂项：记号笔、美纹胶带、扎带/魔术贴（¥50）——免破坏安装与点位可调。用途：① 记号笔用于图纸现场标注；② 美纹胶带/无痕双面胶用于信标试挂（先临时固定、实测覆盖、确认后再定永久位置）；③ 扎带/魔术贴用于不能打孔的部位（门套、管道、桥架、吊顶）固定信标。不采购的后果：现行 61 枚信标的 mountType 为 wall 54 枚 + door_frame 7 枚，绝大多数固定在墙面与门套上，二者均为校方资产不得钻孔；缺免钻孔固定材料则挂高无法稳定保持 2.2 m（语义规范要求全站一致），挂高与朝向漂移会直接改变实测 RSSI 使数据失真、无法与仿真结论对齐；同时「优先微调点位（免费）」的风险应对失去物理前提。¥50 为整卷胶带 + 扎带 + 记号笔的打包估算。", size=9, after=3)
    para(doc, "小结：D 类合计 ¥180，仅占申请总额约 4.5%，但直接决定 ②③④ 三个阶段的作业可行性与数据可信度——D1 保证数据可追溯、D2 保证现场可核对、D3 保证安装不破坏校方设施且点位可调。三者任缺其一，都会使采到的数据无法核对、无法复现，从而使前期投入的测量设备与信标难以产出可验收成果。", size=9, after=4)

    heading(doc, "三、预算合计", 1)
    para(doc, "测距设备 A（¥614）与辅助耗材 D（¥180）为固定项，合计 ¥794；信标 B 按三档 × 两方案计；采集终端与充电使用现有设备，不计入预算。", size=9.5, after=4)
    para(doc, "方案 I（71 枚信标）", size=10, bold=True, after=2)
    make_table(doc,
        ["信标选型", "信标小计(¥)", "合计(¥)"],
        [["档1 Minew E5（¥45 × 71）", "3,195", "3,989"],
         ["档2 E5 国产芯片版（¥35 × 71）", "2,485", "3,279"],
         ["档3 RF-B-SR1（¥27 × 71）", "1,917", "2,711"]],
        widths=[6.0, 3.6, 3.6])
    para(doc, "方案 II（108 枚信标，5m 覆盖风险预案）", size=10, bold=True, after=2)
    make_table(doc,
        ["信标选型", "信标小计(¥)", "合计(¥)"],
        [["档1 Minew E5（¥45 × 108）", "4,860", "5,654"],
         ["档2 E5 国产芯片版（¥35 × 108）", "3,780", "4,574"],
         ["档3 RF-B-SR1（¥27 × 108）", "2,916", "3,710"]],
        widths=[6.0, 3.6, 3.6])
    para(doc, "计算式：合计 = 信标小计 + 794（A+D）；采集终端与充电宝不计入。建议先按方案 I 采购（推荐档3 预算版或档1 Minew），验样 2~3 枚走通配置流程；现场实测有效覆盖半径后，再决定是否按方案 II 追加 37 枚。", size=9, after=4)

    heading(doc, "四、与全量部署（全楼层）方案的预算对比", 1)
    para(doc, "上述方案 I / II 均为路线级部署（沿导航路线与关键区域布点）。若目标升级为「全楼任意位置均可实时定位」，需采用 docs/08 §五 的全楼层部署方案 C：324 枚部署 + 5 备件 = 329 枚（F1 204 + F2 120）。下表为同口径（信标 + A/D 固定项 ¥794，全部使用现有设备）对比。", size=9.5, after=4)
    make_table(doc,
        ["信标选型", "① 方案I（71枚）", "② 方案II（108枚）", "③ 全量（329枚）", "③/①", "①→③追加"],
        [["档1 Minew E5（¥45）", "3,989", "5,654", "15,599", "3.91×", "+11,610（258枚）"],
         ["档2 E5 国产芯片版（¥35）", "3,279", "4,574", "12,309", "3.75×", "+9,030（258枚）"],
         ["档3 RF-B-SR1（¥27）", "2,711", "3,710", "9,677", "3.57×", "+6,966（258枚）"]],
        widths=[3.6, 2.4, 2.4, 2.4, 1.3, 3.0])
    para(doc, "全量部署信标小计（¥14,805 / ¥11,515 / ¥8,883）与 docs/08 §6.2 一致，此处补齐测距设备（¥614）与耗材（¥180）固定项以保证可比。", size=8.5, after=4)
    para(doc, "结论：多花的钱买的是「覆盖范围」，不是「精度」——纯三点在线定位仍为 1.33m（误差主因是 RSSI 噪声与 NLOS 偏置，加密信标无法消除），亚米级仍依赖指纹库 + 滤波融合。建议先按方案 I 完成路线级验证与指纹库建设，确需全楼覆盖时再追加 258 枚（档3 仅 +¥6,966），固定设备与流程可复用、无沉没成本。", size=9, after=4)

    heading(doc, "五、可选升级方案（非必须）", 1)
    make_table(doc,
        ["物品", "规格", "参考价(¥)", "说明"],
    ...
    bullet(doc, "终端与充电：采集终端复用成员现有手机（微信小程序，Android/iOS 均可完成 RSSI 采集，距离真值由测距设备提供），充电宝自备——两项均不列入采购。")
    ...
    bullet(doc, "全量部署对比：全楼层方案 C（324 + 5 = 329 枚，F1 204 / F2 120）与配套预算引自 docs/08 §五、§六，已在同口径下重算，详见第四节。")
    bullet(doc, "参考文档：docs/08 信标部署实施方案（含全楼方案）、docs/13 信标部署质量分析、docs/16 亚米级误差优化、docs/18 语义优化 TODO、fingerprint-collector/README.md 采集小程序说明。")
