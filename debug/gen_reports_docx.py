...
def build_progress_report():
    ...
    bullet(doc, "按当前 61 枚部署 + 10 枚备用 = 71 枚计：激光测距套装约 ¥524；蓝牙信标三档备选 ¥1,917~3,195；辅助耗材 ¥180；采集终端与充电宝使用现有设备，不另购。方案 I 合计约 ¥2,621~3,899。")
...
def build_budget():
    ...
    para(doc, "测距仪 1 台即可：直角坐标法在同一测点上分两次测量（先量沿墙距离、再垂直打墙），一台顺序完成即可，不需同时工作；并与信标现场定位、地图勘测共用（docs/08 §6.3 同一口径）。若后续需双人并行采集或热备份，可再加购一台（+¥169）。京东在售参考价：A1 得力 50m±2mm 测距仪约 ¥150–170；A2 水平刻度环全景云台约 ¥200–400（徕图 LB-60N 约 ¥403）；A3 专用夹具/快装板约 ¥20–40；A4 数魅 S-MT14 迷你三脚架约 ¥77；A5 得力 8210 钢卷尺 10m 约 ¥28。上表单价为预算估算值，实际以下单时为准。", size=9, after=4)
    ...
    para(doc, "小结：D 类合计 ¥180，仅占申请总额约 4.6%，但直接决定 ②③④ 三个阶段的作业可行性与数据可信度——D1 保证数据可追溯、D2 保证现场可核对、D3 保证安装不破坏校方设施且点位可调。三者任缺其一，都会使采到的数据无法核对、无法复现，从而使前期投入的测量设备与信标难以产出可验收成果。", size=9, after=4)

    heading(doc, "三、预算合计", 1)
    para(doc, "测距设备 A（¥524）与辅助耗材 D（¥180）为固定项，合计 ¥704；信标 B 按三档 × 两方案计；采集终端与充电使用现有设备，不计入预算。", size=9.5, after=4)
    para(doc, "方案 I（71 枚信标）", size=10, bold=True, after=2)
    make_table(doc,
        ["信标选型", "信标小计(¥)", "合计(¥)"],
        [["档1 Minew E5（¥45 × 71）", "3,195", "3,899"],
         ["档2 E5 国产芯片版（¥35 × 71）", "2,485", "3,189"],
         ["档3 RF-B-SR1（¥27 × 71）", "1,917", "2,621"]],
        widths=[6.0, 3.6, 3.6])
    para(doc, "方案 II（108 枚信标，5m 覆盖风险预案）", size=10, bold=True, after=2)
    make_table(doc,
        ["信标选型", "信标小计(¥)", "合计(¥)"],
        [["档1 Minew E5（¥45 × 108）", "4,860", "5,564"],
         ["档2 E5 国产芯片版（¥35 × 108）", "3,780", "4,484"],
         ["档3 RF-B-SR1（¥27 × 108）", "2,916", "3,620"]],
        widths=[6.0, 3.6, 3.6])
    para(doc, "计算式：合计 = 信标小计 + 704（A+D）；采集终端与充电宝不计入。建议先按方案 I 采购（推荐档3 预算版或档1 Minew），验样 2~3 枚走通配置流程；现场实测有效覆盖半径后，再决定是否按方案 II 追加 37 枚。", size=9, after=4)

    heading(doc, "四、与全量部署（全楼层）方案的预算对比", 1)
    para(doc, "上述方案 I / II 均为路线级部署（沿导航路线与关键区域布点）。若目标升级为「全楼任意位置均可实时定位」，需采用 docs/08 §五 的全楼层部署方案 C：324 枚部署 + 5 备件 = 329 枚（F1 204 + F2 120）。下表为同口径（信标 + A/D 固定项 ¥704，全部使用现有设备）对比。", size=9.5, after=4)
    make_table(doc,
        ["信标选型", "① 方案I（71枚）", "② 方案II（108枚）", "③ 全量（329枚）", "③/①", "①→③追加"],
        [["档1 Minew E5（¥45）", "3,899", "5,564", "15,509", "3.98×", "+11,610（258枚）"],
         ["档2 E5 国产芯片版（¥35）", "3,189", "4,484", "12,219", "3.83×", "+9,030（258枚）"],
         ["档3 RF-B-SR1（¥27）", "2,621", "3,620", "9,587", "3.66×", "+6,966（258枚）"]],
        widths=[3.6, 2.4, 2.4, 2.4, 1.3, 3.0])
    para(doc, "全量部署信标小计（¥14,805 / ¥11,515 / ¥8,883）与 docs/08 §6.2 一致，此处补齐测距设备（¥524）与耗材（¥180）固定项以保证可比。", size=8.5, after=4)
